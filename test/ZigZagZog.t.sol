...
contract ZigZagZogTest_commitChoices is ZigZagZogTestBase {
    ...
    function test_commit_choices() public {
        assertFalse(game.getPlayerHasCommitted(gameNumber, roundNumber, player1));
        ...
        bytes memory signature = signMessageHash(player1PrivateKey, choicesMessageHash);

        vm.startPrank(player1);
        game.commitChoices(gameNumber, roundNumber, signature);
        vm.stopPrank();

        assertTrue(game.getPlayerHasCommitted(gameNumber, roundNumber, player1));
    }
    ...
    function testRevert_commit_multiple_times() public {
        ...
        assertFalse(game.getPlayerHasCommitted(gameNumber, roundNumber, player1));
        assertFalse(game.getPlayerHasRevealed(gameNumber, roundNumber, player1));
